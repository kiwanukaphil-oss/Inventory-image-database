...
// The Railway API origin is the only browser-side backend configuration.
if (!isRailwayCatalogConfigured) {
  mount.innerHTML = `<div class="auth"><div class="card">
    ...
    <p style="font-size:13px;color:var(--muted)">
      ...
      <code>VITE_CATALOG_API_URL</code>, then rebuild.
    </p>
  </div></div>`;
} else {
  ...
  async function route(session) {
    ...
    try {
      ...
      const renderedProfile = buildRailwayCatalogProfile(profile);
  ...
  // SIGNED_IN / SIGNED_OUT. Defer routing to keep auth notifications isolated
  // from rendering work.
